...
import { getTeam, getCompetition, getTopScorers, TEAMS } from '@/lib/competitions'
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return buildMetadata({
    ...
    description: `${team.name} top scorers and key players in the ${comp.name} 2025/26 season.`,
    canonical: `https://goaltify.com/competitions/${comp.slug}/teams/${team.slug}/top-players`,
    keywords: [`${team.name} top scorers`, `${team.name} players`, `${team.name} goals`],
  })
}

export default async function TeamTopPlayers({ params }: Props) {
  const team = getTeam(params.team)
  const comp = getCompetition(params.competition)
            ...
            <tbody>
              {teamScorers.map((p, i) => (
                <tr key={p.player.id} className="border-b border-gray-800 hover:bg-gray-800/30">
                  ...
                  <td className="text-center py-3 font-bold text-white">
                    {p.statistics[0]?.goals?.total ?? 0}
                  </td>
                  <td className="text-center py-3 text-gray-400">
                    {p.statistics[0]?.assists ?? 0}
                  </td>
                  <td className="text-center py-3 text-gray-400 hidden sm:table-cell">
